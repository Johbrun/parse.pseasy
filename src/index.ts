...
if (PARSE_MODE) {
  const mdBuffer = fs.readFileSync("./md/" + YEAR + "/_global_fixed.md").toString() as string;
  ...
  for (let i = 0; i < sheets.length; i++) {
    ...
    const path = `./md/${YEAR}/${data.reference}.md`;
...
for (let j = 0; j < sheetsPath.length; j++) {
  ...
  if (toLongTitle) {
    console.warn(
      "[" + data.reference + "][CHECK] INCORRECT_TITLE    : " + toLongTitle[0].substring(0, 100).replace(/\s\n/g, " ")
    );
    error = true;
    nbErr++;
  }
